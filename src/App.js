...
rating = (event) => {
  ...
  if(!isLoaded){
    ...
    return (
      <div className="App">
          <h1>Welcome</h1>
          <h2>Loading Data</h2>
          <img src="https://media.giphy.com/media/WiIuC6fAOoXD2/giphy.gif" className="App-logo" alt="logo" />
    ...
    //filters by rating 
  let ratingsfiler = userReviews.filter(
      function(review){
        if(rating == 0){
          return true
        }
  ...
  let userReviewsFilter = ratingsfiler.filter(
      (review) => {
        return (
          review.title.toLocaleLowerCase().indexOf(this.state.search.toLocaleLowerCase()) !== -1 || review.review.toLocaleLowerCase().indexOf(this.state.search.toLocaleLowerCase()) !== -1
        )
      }
    )
  ...
  let todayPost = userReviewsFilter.filter(
    function(review){
      let tempDate = review.date.split("-",3)
      let lastSplit = tempDate[2].split("T")
      if(tempDate[0]== todayYear && tempDate[1]== todayMonth && lastSplit[0] == todayDate){
        return true
      }
      else{
        return false
  ...
  let yesterdayPost = userReviewsFilter.filter(
    function(review){
      let tempDate = review.date.split("-",3)
      let lastSplit = tempDate[2].split("T")
      //most cases fall in this one. where it's not the first of the month
      if(todayDate != 1){
        if(tempDate[0]== todayYear && tempDate[1]== todayMonth && lastSplit[0] == todayDate-1){
          return true
        }
        else{
          return false
      ...
      //if it is the first of the month and it also happens to be on jan 1st then get dec 31st of prev year
        ...
        if(tempDate[0]== todayYear-1 && tempDate[1]== 12 && lastSplit[0] == 31){
          return true
        }
        else{
          return false
      ...
      //when it's the first of the month on feb-dec
        ...
        if(tempDate[0]== todayYear && tempDate[1]== todayMonth-1 && lastSplit[0] == lastDayOfLastMonth){
          return true
        }
        else{
          return false
...
let notYesterdayOrToday = userReviewsFilter.filter( 
  function( element ) {
  return yesterdayPost.indexOf( element ) < 0 && todayPost.indexOf(element) < 0;
} );


  
  //gets the post from this week
  let thisWeek = notYesterdayOrToday.filter(
    function(review){
      let tempDate = review.date.split("-",3)
      ...
      Mondaycal()
        if(Monday < todayDate){
          if(tempDate[0] == todayYear && tempDate[1]== todayMonth && lastSplit[0] >= Monday && lastSplit[0] <= todayDate){
              return true
          }
          else{
            return false
        ...
        // every month but january
          ...
          if(tempDate[0] == todayYear && ((tempDate[1]== todayMonth-1 && lastSplit[0] >= Monday) || (tempDate[1]== todayMonth && lastSplit[0] <= todayDate))){
            return false
        }
          else{
            return false
          ...
          if((tempDate[0] == todayYear-1 && tempDate[1] == todayMonth-1 && lastSplit[0] >= Monday)||(tempDate[0] == todayYear && tempDate[1] == todayMonth && lastSplit[0] <= todayDate)){
            return true
          }
          else return false
        }
  ...
  let lastWeek = notYesterdayOrToday.filter(
    function(review){
      prevSundaycal()
      prevMondayCal()
      let tempDate = review.date.split("-",3)
      ...
      let lastSplit = tempDate[2].split("T")
        if(prevMonday < prevSunday){
          if(tempDate[0] == todayYear && tempDate[1]== todayMonth && lastSplit[0] >= prevMonday && lastSplit[0] <= prevSunday){
              return true
          }
          else{
            return false
        ...
        //every month but january
          ...
          if(tempDate[0] == todayYear && ((tempDate[1]== todayMonth-1 && lastSplit[0] >= prevMonday) || (tempDate[1]== todayMonth && lastSplit[0] <= prevSunday))){
            return true
        }
          else{
            return false
          ...
          if((tempDate[0] == todayYear-1 && tempDate[1] == todayMonth-1 && lastSplit[0] >= prevMonday)||(tempDate[0] == todayYear && tempDate[1] == todayMonth && lastSplit[0] <= prevSunday)){
            return true
          }
          else return false
        }
  ...
  let notThisWeekOrLast = userReviewsFilter.filter( 
    function( element ) {
    return thisWeek.indexOf( element ) < 0 && lastWeek.indexOf(element) < 0 && todayPost.indexOf( element ) < 0 && yesterdayPost.indexOf(element) < 0;
  } );
  
  let thisMonth = notThisWeekOrLast.filter(
    function(review){
      let tempDate = review.date.split("-",3)
      let lastSplit = tempDate[2].split("T")
  ...
  let lastMonth = notThisWeekOrLast.filter(
    function(review){
      let tempDate = review.date.split("-",3)
      let lastSplit = tempDate[2].split("T")
      if(todayMonth != 1){
  ...
  let notThismonthorLast = userReviewsFilter.filter( 
    function( element ) {
    return thisWeek.indexOf( element ) < 0 && lastWeek.indexOf(element) < 0 && todayPost.indexOf( element ) < 0 && yesterdayPost.indexOf(element) < 0 && thisMonth.indexOf( element ) < 0 && lastMonth.indexOf(element) < 0;
  } );
...
//sorting everything that is not in those buckets up ahead
  let notThismonthorLastsorted = notThismonthorLast.sort(
    function (review, review2){
  ...
  for(var index = 0; index < notThismonthorLastsorted.length; index++){
      let dateHolder = notThismonthorLastsorted[index].date.split("-",3)
        ...
        else if(dateHolder[0] == monthYearArray[index-1][0] && dateHolder[1] == monthYearArray[index-1][1]){
          count++;
          monthYearArray.push([[dateHolder[0]],[dateHolder[1]]])
        }
        else{
  ...
  bucket++
    ...
    let userReviewsLimitFilter = userReviewsFilter.filter((x,index) =>  index < this.state.limit)
...
{/*shows the rating */}
  <label>Select by Rating</label>
    <select id="rating" value={this.state.value} onChange={this.rating}> 
     ...
     {todayPost.length > 0 && <Countbucket Month="Today" Year="" array={todayPost}></Countbucket>} 
         
     {yesterdayPost.length > 0 && <Countbucket Month="Yesterday" Year="" array={yesterdayPost}></Countbucket>}  

        {/* {notYesterdayOrToday.length} */}
    {thisWeek.length > 0 && <Countbucket Month="This Week" Year="" array={thisWeek}></Countbucket>} 

     {lastWeek.length > 0 && <Countbucket Month="Last Week" Year="" array={lastWeek}></Countbucket>}

    {thisMonth.length > 0 && <Countbucket Month="This Month" Year="" array={thisMonth}></Countbucket>}  
    
      {lastMonth.length > 0 && <Countbucket Month="Last Month" Year="" array={lastMonth}></Countbucket>} 
  
    
    {placeinbuckets.map(
      function(item,index){
        return <Countbucket Month={monthToString[parseInt(monthYearTracker[index][0][1])]} Year={monthYearTracker[index][0][0]} array={item}></Countbucket>
      }
    )}
